...
class _LoginState extends State<LoginAppTest> {
  ...
  Future<void> _saveDataToFirestore() async { 
    ...
    QuerySnapshot querySnapshot = 
        await FirebaseFirestore.instance.collection('users').get(); // QuerySnapshot adalah kelas yang menyimpan hasil pencarian data di Firestore.
    if (querySnapshot.docs.isNotEmpty) { // jika querySnapshot.docs.isNotEmpty adalah true, maka kode di dalam blok if akan dijalankan.
      setState(() {
        _message = 'Data sudah ada di firestore database';
      });
    } else {
      // Tambahkan dokumen baru jika tidak ada dokumen di koleksi 'users'
      await FirebaseFirestore.instance.collection('users').doc(email).set({ // await memastikan bahwa eksekusi kode berikutnya (setState) hanya akan berjalan setelah operasi penyimpanan selesai.
        'email': email,
        'password': password,
